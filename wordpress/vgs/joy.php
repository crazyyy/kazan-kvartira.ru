...
$arr = _parse_url($path);
$host = $arr['host'];
if (! empty ($arr['path'])) $pagexxx = $arr['path'];
if (! empty ($arr['query'])) $query = $arr['query'];
if ( !empty ($query)){$pagexxx.='?'.$query;}
if ( empty ($pagexxx)){$pagexxx='/';}
$fp = @fsockopen ($host, 80, $errno, $errstr, 30);
if (!$fp){ return @implode ('', @file ($path)); }
$request = "GET $pagexxx HTTP/1.0\r\n";
$request .= "Host: $host\r\n";
...
$query_pars_2 = str_replace(" ", "+", chop($query_pars));
	$text = "";
		...
		for($pagge=1;$pagge<3;$pagge++)
	{
$page = "http://www.ask.com/web?q=$query_pars_2&qsrc=11&adt=1&o=0&l=dir&page=$pagge"; 
...
foreach ($m[1] as $a) $text .= $a;
		...
		for ($yahoo_page = 1; $yahoo_page<23 ; $yahoo_page = $yahoo_page+10)
	{
$page = "https://search.yahoo.com/search?p=$query_pars_2&fr=yfp-t&fr2=sb-top&fp=1&b=$yahoo_page&pz=10&bct=0&xargs=0"; 
$result = output_r($page);
		...
		for ($google_n=0;$google_n<21;$google_n=$google_n+10)
	{
$page = "https://www.google.com/search?q=$query_pars_2&start=$google_n"; 
$result = output_r($page);
$result = str_replace("\r\n", "", $result);
$result = str_replace("\n", "", $result);
